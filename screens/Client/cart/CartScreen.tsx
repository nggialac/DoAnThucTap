...
const CartScreen = ({ navigation }) => {
  ...
  const changeNumberOfProduct = (manhathuoc: string, masp: string, soluong: number) => {
  ...
  const CartCard = ({ item, index }) => {
    return (
      <View style={style.cartCard}>
        ...
        >
          <Text style={{ fontWeight: "bold", fontSize: 16 }}>{item.sanpham.tensp}</Text>
        ...
        <View style={{ marginRight: 20, alignItems: "center" }}>
          <Text style={{ fontWeight: "bold", fontSize: 18 }}>{item.soluong}</Text>
        ...
        ListFooterComponent={() => (
          <View>
            ...
            <View style={{ marginHorizontal: 30 }}>
              <PrimaryButton title="CHECKOUT" onPress={()=>{navigation.navigate("CheckOutScreen", {total})}}/>
            </View>
          </View>
        )}
      />
